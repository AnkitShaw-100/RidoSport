                    ...
                    <section>
                        <header>
                            ...
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                {{ __('Provide the necessary details for the new service.') }}
                            </p>
                        </header>

                        <form action="{{ route('service-list.store') }}" method="POST" id="service-form" class="mt-6 space-y-6">
                            @csrf

                            <!-- Main Service Name -->
                            <div>
                                <x-input-label for="service-name" :value="__('Main Service Name')" />
                                <x-text-input id="service-name" name="service_name" type="text" class="mt-1 block w-full" required />
                                <x-input-error class="mt-2" :messages="$errors->get('service_name')" />
                            </div>

                            <!-- Custom Slug -->
                            <div>
                                <x-input-label for="service-slug" :value="__('Custom Slug (optional)')" />
                                <x-text-input id="service-slug" name="slug" type="text" class="mt-1 block w-full" placeholder="Leave empty to auto-generate" />
                                <x-input-error class="mt-2" :messages="$errors->get('slug')" />
                            </div>

                            <!-- Custom URL -->
                            <div>
                                <x-input-label for="service-url" :value="__('Custom URL (optional)')" />
                                <x-text-input id="service-url" name="url" type="text" class="mt-1 block w-full" placeholder="Leave empty to auto-generate from slug" />
                            ...
                            <div class="flex items-center gap-4 mt-4">
                                ...
                                <a href="{{ route('service-list.index') }}" class="inline-block px-4 py-2 text-sm font-semibold text-gray-900 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-600">{{ __('Cancel') }}</a>
                            </div>
                        </form>
                    </section>
                </div>
