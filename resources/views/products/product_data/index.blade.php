        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <a href="{{ route('product-data.create') }}" class="btn btn-primary mb-4">Add New Product</a>
                        ...
                        <tbody>
                            @foreach ($products as $index => $product)
                                <tr class="border-b border-black">
                                    ...
                                    <td class="p-2 border-r border-black">{{ $product->subproduct->name }}</td>
                                    <td class="p-2 border-r border-black">{{ $product->name }}</td>

                                    <td class="p-2 border-r border-black">
                                        <img src="{{ url('/' . $product->image) }}" alt="product_image" style="width:300px">
                                    </td>
                                    ...
                                    <td class="p-2 border-r border-black">
                                        <a href="{{ route('product-data.edit', $product->id) }}" class="btn btn-warning">
                                            <i class="fas fa-pencil"></i></a>
                                        <form action="{{ route('product-data.destroy', $product->id) }}" method="POST" style="display: inline-block;">
                                            @csrf
                                            @method('DELETE')
                    ...
                    {{-- Pagination Links --}}
                    <div class="mt-4">
                        {{-- {{ $products->links() }}  <!-- Laravel's built-in pagination links --> --}}
                    </div>
                </div>
            </div>
        </div>
