...
//     // Delay for 200ms to ensure everything has had time to load
...
//   }
  ...
  if ($('.feature-block-three').length) {
    ...
    $($service_block).on('mouseenter', function () {
      $(this).find('.content-box .inner').stop().slideDown(400);
      return false;
    });

    $($service_block).on('mouseleave', function () {
  ...
  // One-page Navigation Activation
    ...
    if ($onepage_nav.length > 0) {
      ...
      $sections.each(function () {
        ...
        if (cur_pos >= top && cur_pos <= bottom) {
          ...
          $onepage_nav.find('a[href="#' + $(this).attr('id') + '"]').parent().addClass('current active');
        }

        if (cur_pos <= nav_height && cur_pos >= 0) {
          $onepage_nav.find('a').parent().removeClass('current active');
          $onepage_nav.find('a[href="#header"]').parent().addClass('current active');
        }
      });
    }
  }
